...
class MCANet(nn.Module):
    def __init__(self, dec_in, c_out, seq_len, label_len, out_len,
                 ...
                 decomp_kernel=[33], conv_kernel=[12, 24], isometric_kernel=[18, 6], top_k=3):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if self.mode == 'regre':
            seasonal_init_enc = self.rev(x_enc, 'norm')
            trend = self.regression(x_enc.permute(0, 2, 1)).permute(0, 2, 1)

        elif self.mode == 'mean':
            mean = torch.mean(x_enc, dim=1).unsqueeze(1).repeat(1, self.pred_len, 1)
            seasonal_init_enc, trend = self.decomp_multi(x_enc)
            seasonal_init_enc = self.rev(seasonal_init_enc, 'norm')
            trend = torch.cat([x_enc[:, -self.seq_len:, :], mean], dim=1)
